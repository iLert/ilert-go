...
- add `GetCurrentAccount` behind `/api/tenants/current`, with the `Account` and `AccountSubscription` types and the `AccountLanguage`, `AiMode` and `SubscriptionStatus` constants. `Account.ApplicationFeatures` is deliberately not mirrored as a constant list: the API documents the set as growing with every feature ilert ships and not part of the stable contract. `Account.AllowAI` is returned by the API but missing from the published spec, and is included here (verified against the API on 16.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
- add the label discovery endpoints for the three entities that carry labels: `GetAlertLabelKeys`/`GetAlertLabelValues`, `GetServiceLabelKeys`/`GetServiceLabelValues` and `GetTelemetrySourceLabelKeys`/`GetTelemetrySourceLabelValues`, with the shared `LabelKey` and `LabelValue` types. They list the distinct label keys and the values used for one key, which is what a label filter has to be built from [#82](https://github.com/iLert/ilert-go/pull/82)
- the label endpoints take `StartIndex` and `MaxResults`, and the SDK only sends what the caller set: the two families page differently and an invented default would break one of them. The alert label endpoints enforce a fixed page size, rejecting any `max-results` other than 100 and any `start-index` that is not a multiple of it with a 400, exposed as `AlertLabelPageSize`. The service and telemetry source ones page freely up to `LabelMaxResults` (1000), so the spec text about a fixed page size only holds for alerts (verified against the API on 16.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
- `GetAlertLabelKeys` and `GetAlertLabelValues` read through a cache when no `Query` is set: five minutes for the first three pages of the keys, two minutes for the first page of the values. The labels of a freshly created alert can take that long to show up, and an account whose labels were listed while it had none keeps answering with an empty list until the entry expires. Setting `Query` bypasses the cache. Documented on the methods, since it is easy to mistake for a broken endpoint [#82](https://github.com/iLert/ilert-go/pull/82)
- `GetAlertLabelKeys` and `GetAlertLabelValues` return an empty list to callers with the guest or stakeholder role. The API answers them with an empty object where every other label endpoint answers with an empty list, and the SDK reads that as no labels instead of failing to decode it (verified against the API on 25.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
- add `GetServiceTopology` behind `/api/services/topology`, with the `ServiceGraph` and `ServiceNode` types. Its `Labels` input is a single condition expression (`==`, `!=`, `in`, `not_in`, combined with `and`), the same one the API takes for the labels filter of its service, alert and telemetry source lists, so it is typed as a `*string` rather than a slice. `ServiceGraph.Edges` is the rendering projection of the dependencies and only carries the edge id, both service ids and the type; the full edge still comes from `GetServiceDependency`. The endpoint is gated behind the `TOPOLOGY` tenant feature and answers 402 `FEATURE_REQUIRED` without it [#82](https://github.com/iLert/ilert-go/pull/82)
- add `PublishServiceStatus` behind `/api/services/{id}/publish-status`, which makes the latest status change of a service visible on status pages when it was kept internal. Such a change is recognizable by a non-empty `PublicStatus`, read through `ServiceInclude.PublicStatus`: the API only returns it while it differs from `Status`. The required `Status` input guards against publishing a status change that happened in the meantime: the API answers 409 and publishes nothing when it does not match the current status, and answers 409 as well when there is no internal change to publish. Note that the client classifies a 409 as a transient conflict and retries it before surfacing it as a `*RetryableAPIError`, and that the service returned by the endpoint carries no `PublicStatus`, so the published status has to be read back through `ServiceInclude.PublicStatus` (verified against the API on 16.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
- add the chat channel messages behind `/api/chat-channels/{channelId}/messages`: `GetChatChannelMessages`, `GetChatChannelMessage`, `CreateChatChannelMessage`, `UpdateChatChannelMessage` and `DeleteChatChannelMessage`, with the `ChatChannelMessage` type and the `ChatChannelType`, `ChatChannelMessageAuthorType`, `ChatChannelMessageContentType` and `ChatChannelMessageStyle` constants. For the channel type `ALERT` the channel id is the id of the alert, and the author of anything written through these endpoints is the user the API key or access token belongs to [#82](https://github.com/iLert/ilert-go/pull/82)
- add the thread replies behind the same channel: `GetChatChannelThreadReplies`, `GetChatChannelThreadReply`, `CreateChatChannelThreadReply`, `UpdateChatChannelThreadReply` and `DeleteChatChannelThreadReply`, with the `ChatChannelThreadReply` type. A message becomes a thread with its first reply [#82](https://github.com/iLert/ilert-go/pull/82)
- add the reactions on both: `AddChatChannelMessageReaction`, `RemoveChatChannelMessageReaction`, `AddChatChannelThreadReplyReaction` and `RemoveChatChannelThreadReplyReaction`, with the `ChatChannelMessageReaction` type. They are their own endpoints rather than a field on the message, they always answer with the updated message or reply, and reacting twice with the same code is rejected with a 400 [#82](https://github.com/iLert/ilert-go/pull/82)
- `DeleteChatChannelMessage` and `DeleteChatChannelThreadReply` return the deleted entity instead of an empty output, unlike every other delete in this SDK. The API deletes softly: the entity stays in the list with `Deleted` set and its content replaced by a `<Deleted on ...>` placeholder, and it answers 200 with that body rather than 204 (verified against the API on 17.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
- every chat operation sends the required `channel-type` query parameter. The API answers `400 channelId and channelType must not be null` without it, so it is an input on every one of them rather than an optional filter [#82](https://github.com/iLert/ilert-go/pull/82)
- `ChatChannelMessage.MessageStyle` is worth checking before rendering a message in your own interface. ilert uses styles other than `DEFAULT` internally, for example for the intermediate steps of its AI agents, and the API documents that only `DEFAULT` should be displayed [#82](https://github.com/iLert/ilert-go/pull/82)
- add `GetCallFlowSessions`, `GetCallFlowSession` and `GetCallFlowSessionData` behind `/api/call-flow-sessions`, with the `CallFlowSession` type and the `CallFlowSessionStatus` constants. A session is one inbound call handled by a call flow, carrying the caller location, the user the call was connected to and the link to a recorded voicemail [#82](https://github.com/iLert/ilert-go/pull/82)
- `GetCallFlowSessionsInput.State` is a single value rather than a slice, even though the API takes a repeatable `states` parameter: it documents a maximum of one entry and does not reject more, it silently answers with an empty list, so a slice would only invite a filter that never matches [#82](https://github.com/iLert/ilert-go/pull/82)
- `GetCallFlowSessionData` returns a `map[string]any`. The keys are whatever the nodes of the call flow collected during the call, for example a pin or a gathered answer, and the API documents no fixed schema for them [#82](https://github.com/iLert/ilert-go/pull/82)
