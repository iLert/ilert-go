...
- add `GetServiceTopology` behind `/api/services/topology`, with the `ServiceGraph` and `ServiceNode` types. Its `Labels` input is a single condition expression (`==`, `!=`, `in`, `not_in`, combined with `and`), not the repeated `key:value` filter of the list endpoints, so it is typed as a `*string` rather than a slice. `ServiceGraph.Edges` is the rendering projection of the dependencies and only carries the edge id, both service ids and the type; the full edge still comes from `GetServiceDependency`. The endpoint is gated behind the `TOPOLOGY` tenant feature and answers 402 `FEATURE_REQUIRED` without it [#82](https://github.com/iLert/ilert-go/pull/82)
- add `PublishServiceStatus` behind `/api/services/{id}/publish-status`, which makes the latest status change of a service visible on status pages when it was kept internal. Such a change is recognizable by a `PublicStatus` that differs from `Status`, both read through `ServiceInclude.PublicStatus`. The required `Status` input guards against publishing a status change that happened in the meantime: the API answers 409 and publishes nothing when it does not match the current status. Note that the client classifies a 409 as a transient conflict and retries it before surfacing it as a `*RetryableAPIError`, and that the service returned by the endpoint carries no `PublicStatus`, so the published status has to be read back through `ServiceInclude.PublicStatus` (verified against the API on 16.09.2026) [#82](https://github.com/iLert/ilert-go/pull/82)
...
- `GetCallFlowSessionsInput.State` is a single value rather than a slice, even though the API takes a repeatable `states` parameter: it documents and enforces a maximum of one entry, so a slice would only invite a 400 [#82](https://github.com/iLert/ilert-go/pull/82)
